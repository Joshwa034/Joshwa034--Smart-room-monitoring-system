...
import os
import time
# import the required MySQL libraries
...
while True:
    ...
    for contour in contours:
        ...
        if capture_time is not None and time.time() - capture_time < 10 and capture_count < 5 and time.time() - last_capture_time >= 2:
            filename = "http://localhost/images/image{}_{}.jpg".format(capture_count, int(time.time()))
            cv2.imwrite(filename, frame)

            # insert the image and capture time in the database
            sql = "INSERT INTO images (capture_time, image_path) VALUES (%s, %s)"
            val = (time.strftime('%Y-%m-%d %H:%M:%S'), filename)
